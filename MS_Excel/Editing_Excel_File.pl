...
my $workbook = $parser->parse('perl.xlsx');
...
for my $worksheet ( $workbook->worksheet(0) ) {
    ...
    for my $row ( $row_min .. $row_max ) {
        for my $col ( $col_min .. $col_max ) {
            ...
            my $cell =
              $worksheet->get_cell( $row, $col );    # Get current cell value
            next
              unless $cell;  # Move to next loop element unless $cell is defined
...
#$read_cell_value =~ s/\W//g; # Remove everything except alphanumeric characters and underscores
            $read_cell_value =~
              s/[#\%&\*+()]//g;    # Removing select special charecters

            $worksheet_out->write( $row, $col, $read_cell_value )
