...
$mw->title('Ashes Note Keeper - Version 1.0');
...
my $label_fill_1 = $mw->Label( -width => '15' )
  ->grid( -row => 1, -column => 27, -columnspan => 15, -sticky => 'w' );
...
#Send contents of entry boxes to a file
$mw->Button( -text => 'Select Log File', -width => 14, -command => sub { } )
  ->grid( -row => 24, -column => 26, -columnspan => 14, -sticky => 'w' );

#Create A New File
$mw->Button( -text => "Create", -width => 10, -command => sub { } )
  ->grid( -row => 24, -column => 34, -columnspan => 10, -sticky => 'w' );

#Select a current log file
$mw->Button( -text => "Edit", -width => 10, -command => sub { } )
  ->grid( -row => 24, -column => 40, -columnspan => 10, -sticky => 'w' );

#refresh document in current side view
$mw->Button( -text => "Delete", -width => 10, -command => sub { } )
  ->grid( -row => 24, -column => 46, -columnspan => 10, -sticky => 'w' );
...
my $label_area_1 = $mw->Label( -text => "$widgit_1_lable" )
  ->grid( -row => 1, -column => 4, -columnspan => 5, -sticky => 'w' );
my $lb = $mw->Scrolled(
    "Listbox",
    -selectmode => "single",
    -width      => 20,
...
#Text Area for notes
my $text_area_lable = $mw->Label( -text => "$widgit_7_lable" )
  ->grid( -row => 13, -column => 1, -columnspan => 5, -sticky => 'w' );
my $text_area2 =
...
my $button4 = $mw->Button(
    ...
    -width   => 25,
    -command => sub { &add_new }
)->grid( -row => 24, -column => 2, -columnspan => 25, -sticky => 'w' );
...
#Exit button
$mw->Button( -text => "Exit", -width => 25, -command => sub { exit } )
  ->grid( -row => 25, -column => 2, -columnspan => 25, -sticky => 'w' );
